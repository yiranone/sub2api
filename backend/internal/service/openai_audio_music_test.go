...
func TestOpenAIGatewayServiceForwardMusic_APIKeyMiniMaxMappedModel(t *testing.T) {
	...
	require.Equal(t, "https://api.minimaxi.com/v1/music_generation", upstream.lastReq.URL.String())
	require.Equal(t, "music-01", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, "url", gjson.GetBytes(upstream.lastBody, "output_format").String())
	require.Equal(t, "short piano theme\nshort piano theme", gjson.GetBytes(upstream.lastBody, "lyrics").String())
