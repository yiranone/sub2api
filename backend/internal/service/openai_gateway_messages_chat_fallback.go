...
func (s *OpenAIGatewayService) streamChatCompletionsAsAnthropic(
	...
	emitChunk := func(chunk *apicompat.ChatCompletionsChunk) {
		...
		responsesEvents := apicompat.ChatCompletionsBridgeChunkToResponsesEvents(chunk, ccState)
	...
	finalEvents := apicompat.FinalizeChatCompletionsBridgeResponsesStream(ccState)
