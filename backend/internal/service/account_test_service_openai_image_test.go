...
func TestAccountTestService_OpenAIImageAPIKeyMiniMaxMappedModelUsesImageEndpoint(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/1/test", nil)
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{"data":{"image_base64":"aGVsbG8="}}`)),
		},
	}
	svc := &AccountTestService{
		httpUpstream: upstream,
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"model_mapping": map[string]any{
				"gpt-image-1": "image-01",
			},
		},
	}

	err := svc.testOpenAIAccountConnection(c, account, "gpt-image-1", "draw a cat", "")
	require.NoError(t, err)
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, "https://api.minimaxi.com/v1/image_generation", upstream.lastReq.URL.String())
	require.Equal(t, "image-01", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, "draw a cat", gjson.GetBytes(upstream.lastBody, "prompt").String())
	require.Contains(t, rec.Body.String(), "data:image/png;base64,aGVsbG8=")
	require.Contains(t, rec.Body.String(), "\"success\":true")
}

func TestAccountTestService_OpenAIImageAPIKeyMiniMaxURLResponseEmitsURL(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/1/test", nil)
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{"data":{"image_urls":["https://cdn.example.com/generated.png"]}}`)),
		},
	}
	svc := &AccountTestService{
		httpUpstream: upstream,
	...
	err := svc.testOpenAIAccountConnection(c, account, "gpt-image-1", "draw a cat", "")
	require.NoError(t, err)
	require.Contains(t, rec.Body.String(), `"image_url":"https://cdn.example.com/generated.png"`)
	require.NotContains(t, rec.Body.String(), "data:image/png;base64,https://cdn.example.com/generated.png")
	require.Contains(t, rec.Body.String(), "\"success\":true")
}
